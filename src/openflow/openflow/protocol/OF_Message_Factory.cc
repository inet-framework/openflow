...
#include <openflow/openflow/protocol/OF_Message_Factory.h>

namespace openflow {

OFP_Features_Reply* OF_Message_Factory::createFeaturesReply(std::string dpid,
...
OFP_Features_Request* OF_Message_Factory::createFeatureRequest() {

    OFP_Features_Request *featuresRequest = new OFP_Features_Request(
            "FeaturesRequest");
...
OFP_Flow_Mod* OF_Message_Factory::createFlowModMessage(
        ofp_flow_mod_command mod_com, const oxm_basic_match& match,
        int pritority, uint32_t* outports, int n_outports, uint32_t idleTimeOut,
    ...
    msg->setFlags(0); //TODO add flags to method signature
    msg->setCookie(42); //TODO add cookie to method signature
...
OFP_Hello* OF_Message_Factory::createHello() {
...
OFP_Packet_In* OF_Message_Factory::createPacketIn(ofp_packet_in_reason reason,
        EthernetIIFrame *frame, uint32_t buffer_id, bool sendFullFrame) {
...
OFP_Packet_Out* OF_Message_Factory::createPacketOut(uint32_t* outports,
        int n_outports, int in_port, uint32_t buffer_id,
    ...
    if (buffer_id == OFP_NO_BUFFER) {   //No Buffer so send full frame.
        if (frame) {
            ...
            throw cRuntimeError(
                    "OFP_NO_BUFFER was set but no frame was provided.");
        }
    } else {
        msg->setIn_port(in_port);
    }
